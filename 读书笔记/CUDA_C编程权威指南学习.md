...
- 线程块大小的影响：
  - 小线程块：在达到每个SM的线程束数限制时，硬件资源未被充分利用。
...
__device__ void __synchthreads(void);
~~~
...
<img src="C:/Users/Lenovo/Desktop/%E9%B1%BC%E5%A7%AC%E7%8E%84%E7%9A%84%E4%B8%9C%E8%A5%BF/Typora%E5%9B%BE%E7%89%87/Streaming%20Multiprocessors%E6%9E%B6%E6%9E%84.png" alt="GP100" style="zoom:67%;" />
...
- **本地内存和全局内存用的是同一块存储区域**，高延迟低带宽。本地内存数据存储在每个SM的一级缓存和每个设备的二级缓存中。
...
- Host虚拟内存机制导致GPU无法安全地访问可分页内存空间。所以，实际的数据传输过程是：CDUA驱动程序首先分配临时页面固定的主机内存，将host source data复制到 **固定内存** 中，然后从固定内存传输到device Mem。
...
- **零拷贝内存**：固定内存，该内存会映射到设备地址空间中。
  ...
  - 所谓映射就是纯硬件方式的拷贝，要走PCIe总线，是很慢的。只适合少量数据的共享。
...
- 对于同一线程束，并发读同一地址，被读取的字会被广播到请求线程。并发写的字，只会被一个线程写入，该线程不是确定的。
...
- 常量内存位于设备的DRAM上，且有一个专用的片上缓存。每个SM**常量缓存**大小的限制时64KB。
...
- 异步数据传输
  - 必须使用固定主机内存：cudaMallocHost和cudaHostAlloc。
